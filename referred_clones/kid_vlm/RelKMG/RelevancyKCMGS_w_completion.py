...
inputPath = "/scratch/rahul.garg/conceptNet/allCmgs_llava_targets_h2.jsonl"
outputName = "allCmgs_llava_Target_h2_RelevancyKCMGS"
if start_line == 0:
...
def get_LM_score(nodes, context):
    nodes = [{"key": "QAcontext", "score": 0}] + nodes[
        :
    ...
    # get mainEmbed
    singleton = torch.tensor(TOKENIZER.encode(context.lower(), add_special_tokens=True)).cuda(gpu_id).unsqueeze(0)
    ...
    while cur_idx < len(sents):
        # Prepare batch
        input_ids = [
            sents[i] for i in range(cur_idx, min(cur_idx + batch_size, len(sents)))
        ]
        max_len = max(len(seq) for seq in input_ids)
        input_ids = [
    ...
    for node in nodes:
        if "score" in node:
            # print("Already has score", node)
            node["score"] = float(node["score"])
            continue
        
        node["score"] = float(scores[score_idx])
        score_idx += 1
