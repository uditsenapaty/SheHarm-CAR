...
os.environ['TORCH_HOME'] = '/staging/users/tpadhi1/rahul/tmp/torchCache'
...
wandb.login(key='aecd26bbfad3adddcb517532f258de44a29b6cc2')
captions = {}

with open('../newCaptionsLLAVA.json', 'r') as file:
    fileObj = json.load(file)
    for key in fileObj:
        # remove dot and after
        captions[int(key.split('.')[0])] = fileObj[key]
...
graphPaths = '/staging/users/tpadhi1/rahul/tmp/gnnCache_llava_target_caption_750_h1/'   # contains id.pkl files
...
graphFile = open('/staging/users/tpadhi1/rahul/llava_Target_and_caption_minilm_h1RelKmgsTop750_noEmbed_hateful.jsonl',
                 'r', encoding='utf-8')
...
class CLIPClassifier(nn.Module):
    def __init__(self, gnnHidden=256, gnnOutput=1024, ptFusion='align', graphFusion='bilinear', fusionOutput=1024,
                ...
                rel2idx=rel2idx, lossAlpha=0.5, trainEncoder=False, consistStrat='post'):
        super().__init__()
        ...
        #         "fusion": self.fusion,
        #         "num_pre_output_layers": self.num_pre_output_layers,
        #         "lr": self.lr,
        #         "weight_decay": self.weight_decay,
        #         "weight_image_loss": self.weight_image_loss,
        #         "weight_text_loss": self.weight_text_loss,
        #         "weight_fine_grained_loss": self.weight_fine_grained_loss,
        #         "weight_super_loss": self.weight_super_loss,
        ...
        # self.post_text2 = nn.Sequential(
        #     nn.ReLU(),
        #     nn.Linear(self.map_dim, self.map_dim)
        # )
    
        self.output = nn.Linear(self.fusionOutput, 1)
    ...
    def forward(self, batch, batch_idx):
        ...
        logits = self.output(finalCombined)
        logits = logits.squeeze(dim=1) # [batch_size, 1(or)n]
        ...
        mainLoss = self.cross_entropy_loss(logits, batch['labels'].float().squeeze())
        # normalize
        output['loss'] =  mainLoss * (1 - self.lossAlpha) + consistency_loss * self.lossAlpha
        output['mainLoss'] = mainLoss
        output['consistencyLoss'] = consistency_loss
        output['preds'] = preds
        output['probs'] = preds_proxy
        return output
...
class HatefulMemesDataset(Dataset):
    def __init__(self, root_folder, image_folder, split='train', image_size=224):
        ...
        # print(f"Original size: {len(self.df)}", flush=True)
        self.df = self.df[self.df['id'].isin(scenes.keys())]
        # print(f"Filtered size: {len(self.df)}", flush=True)
    ...
    def __getitem__(self, idx):
        ...
        # if self.captionBool:
        #     item['text'] = row['text'] + ' . ' + captions[int(row['id'])]
        # else:
        item['text'] = row['text']
        item['caption'] = captions[int(row['id'])]

        item['label'] = row['label']
        item['graph'] = scenes[int(row['id'])]
        item['id'] = int(row['id'])
...
def train(model, optimizer, train_loader, val_loader, n_epochs, model_name=f'best_model_{device_id}.pth', trial=None):
    best_auc = 0
    bestStats = None
    for epoch in range(n_epochs):
        ...
        for batch_idx, batch in tqdm(enumerate(train_loader), total=len(train_loader), desc=f"Epoch {epoch}"):
            ...
            wandb.log({"Main Train Loss": output['mainLoss'].item()})
            wandb.log({"Consistency Train Loss": output['consistencyLoss'].item()})
        ...
        with torch.no_grad():
            ...
            if auc > best_auc:
                best_auc = auc
                bestStats = (acc, f1, auc)
                torch.save(model.state_dict(), model_name)
            print(f"Epoch {epoch}, Val Acc: {acc}, Val F1: {f1} Val AUC: {auc}", flush=True)
            wandb.log({"Val Acc": acc, "Val F1": f1, "Val AUC": auc})
        
        if trial is not None:
            trial.report(best_auc, epoch)
            if trial.should_prune():
                wandb.log({"Pruned": True})
                wandb.log({"Best Val Acc": bestStats[0], "Best Val F1": bestStats[1], "Best Val AUC": bestStats[2]})
...
def evaluate(model, test_loader, mainLog=True):
    ...
    with torch.no_grad():
        ...
        if mainLog:
            wandb.log({"Test AUC": roc_auc_score(all_labels, all_probs), "Test Acc": accuracy_score(all_labels, all_preds), "Test F1": f1_score(all_labels, all_preds)})
        else:
            wandb.log({"Initial Test AUC": roc_auc_score(all_labels, all_probs), "Initial Test Acc": accuracy_score(all_labels, all_preds), "Initial Test F1": f1_score(all_labels, all_preds)})
...
def objective(trial):
    ...
    set_seed(42)
    wandb.init(project="hateClipperOptuna_new_Target_minilm_onlySeen_distil_hop1", mode="online", name=f"{device_id}_{trial.number}", 
    dir='/staging/users/tpadhi1/rahul/tmp')
#  def __init__(self, gnnHidden=256, gnnOutput=1024, ptFusion='align', graphFusion='bilinear', fusionOutput=1024,
#                 hanDim=1024, mulDim=1024, map_dim=1024, lr=1e-4, drop_probs=[0.1, 0.4, 0.2], num_mapping_layers=1, num_pre_output_layers=1, weight_decay=1e-4,
#                 rel2idx=rel2idx):
    

    gnnHidden = trial.suggest_int("gnnHidden", 2, 512)
    gnnOutput = trial.suggest_int("gnnOutput", 2, 1024)
    # ptFusion = trial.suggest_categorical("ptFusion", ['align', 'align_shuffle', 'concat', 'cross', 'cross_nd', 'align_concat'])
    ...
    lossAlpha = trial.suggest_float("lossAlpha", 0, 1)
    ...
    model = CLIPClassifier(gnnHidden=gnnHidden, gnnOutput=gnnOutput, ptFusion=ptFusion, graphFusion=graphFusion, fusionOutput=fusionOutput,
                hanDim=hanDim, mulDim=mulDim, map_dim=map_dim, lr=lr, drop_probs=drop_probs, num_mapping_layers=num_mapping_layers, num_pre_output_layers=num_pre_output_layers, weight_decay=weight_decay, lossAlpha=lossAlpha, trainEncoder=learnEncoders, consistStrat=consistStrat
                ).to(device)

    optimizer = model.configure_optimizers()

    rootData = '/staging/users/tpadhi1/rahul/hateful_memes'
    imFolder = '/staging/users/tpadhi1/rahul/hateful_memes/img'

    train_dataset = HatefulMemesDataset(rootData, imFolder, split='train')
    val_dataset = HatefulMemesDataset(rootData, imFolder, split='dev_seen')
    test_dataset = HatefulMemesDataset(rootData, imFolder, split='test_seen')
...
# objective(None)
study_name = 'hateclipper_rgcn_750_new_target_fixed_distil_h1_minilm'
