...
os.makedirs("h2_intensity_max", exist_ok=True)
os.makedirs("h2_target_max", exist_ok=True)
...
class CLIPClassifier(nn.Module):
    def __init__(self, gnnHidden=256 , weight = None, gnnOutput=1024, ptFusion='align', graphFusion='bilinear', fusionOutput=1024,
        ...
        # )
        self.output_intensity = nn.Linear(output_input_dim, 3)
        if weight is not None:
            self.intensity_cross_entropy_loss = torch.nn.CrossEntropyLoss(reduction='mean', weight=weight)
        else:   
            print("Weight is not none")
            self.intensity_cross_entropy_loss = torch.nn.CrossEntropyLoss(reduction='mean')
    ...
    def forward(self, batch, batch_idx):
        ...
        logits_intensity = self.output_intensity(features_pre_output)  # [batch_size, 3] # Example thresholds
        preds_intensity_proxy = torch.softmax(logits_intensity, dim=1)
        preds_intensity = torch.argmax(preds_intensity_proxy, dim=1)

        mainLoss1 = self.intensity_cross_entropy_loss(logits_intensity, batch['intensity_labels'].long())
        
        output['loss'] =  mainLoss1 * (self.lossAlpha1) + consistency_loss * self.lossBeta1  
        output['mainLoss1'] = mainLoss1
        output['consistencyLoss'] = consistency_loss
        output['preds_intensity'] = preds_intensity
        output['probs_intensity'] = preds_intensity_proxy
        return output
...
class CustomCollator(object):
    ...
    def __call__(self, batch):
        ...
        intensity_labels = torch.LongTensor([item['intensity_label'] for item in batch])
...
def train(model, optimizer, train_loader, val_loader, n_epochs, model_name=f'hateclipper_hop2_KD.pth', trial=None):
    best_acc_intensity = 0
    bestStats_intensity = None
    for epoch in range(n_epochs):
        ...
        for batch_idx, batch in tqdm(enumerate(train_loader), total=len(train_loader), desc=f"Epoch {epoch}"):
            ...
            optimizer.step()
            wandb.log({"Main Train Loss Intensity": output['mainLoss1'].item()})
            wandb.log({"Consistency Train Loss": output['consistencyLoss'].item()})
            
        model.eval()
        with torch.no_grad():
            all_preds_intensity = []
            all_labels_intensity = []
            all_probs_intensity = []
            for batch_idx, batch in enumerate(val_loader):
                ...
                output = model(batch, batch_idx)
                all_preds_intensity.extend(output['preds_intensity'].cpu().numpy())
                all_labels_intensity.extend(batch['intensity_labels'].cpu().numpy())
                all_probs_intensity.extend(output['probs_intensity'].cpu().numpy())
            
            acc_intensity = accuracy_score(all_labels_intensity, all_preds_intensity)
            f1_intensity = f1_score(all_labels_intensity, all_preds_intensity, average='weighted')
            if acc_intensity >= best_acc_intensity:
                best_acc_intensity = acc_intensity
                bestStats_intensity = (acc_intensity, f1_intensity)
                torch.save(model.state_dict(), f"intensity_{model_name}")
            print(f"Epoch {epoch}, Val Acc Intensity: {acc_intensity}, Val F1 Intensity: {f1_intensity}", flush=True)
            wandb.log({"Val Acc Intensity": acc_intensity, "Val F1 Intensity": f1_intensity})

        if trial is not None:
            trial.report(best_acc_intensity , epoch)
            if trial.should_prune():
                wandb.log({"Pruned": True})
                wandb.log({"Best Val Accuracy Intensity": bestStats_intensity[0], "Best Val F1 Intensity": bestStats_intensity[1]})
                wandb.finish()
                raise optuna.TrialPruned()
            
    wandb.log({"Best Val Accuracy Intensity": bestStats_intensity[0], "Best Val F1 Intensity": bestStats_intensity[1]})

    return best_acc_intensity 
...
def evaluate(model, test_loader, mainLog=True):
    ...
    all_labels_intensity = []
    all_probs_intensity = []
    model.load_state_dict(torch.load("intensity_hateclipper_hop2_KD.pth"))
    model.eval()
    ...
    with torch.no_grad():
        ...
        acc_intensity = accuracy_score(all_labels_intensity, all_preds_intensity)
        f1_intensity = f1_score(all_labels_intensity, all_preds_intensity, average='weighted')
        precision_intensity = precision_score(all_labels_intensity, all_preds_intensity, average='weighted')
        recall_intensity = recall_score(all_labels_intensity, all_preds_intensity, average='weighted')
        
        wandb.log({"Test Acc Intensity": acc_intensity, "Test F1 Intensity": f1_intensity})
        wandb.log({"Test Pecision Intensity":precision_intensity, "Test Recall Intensity": recall_intensity})
        print(classification_report(all_labels_intensity, all_preds_intensity, digits=4))
        print(f"Test Acc Intensity: {acc_intensity}, Test F1 Intensity: {f1_intensity} , Test Prcision Intensity: {precision_intensity}, Test Recall Intensity: {recall_intensity}")
...
def calculate_class_weights(train_loader):
    ...
    for batch in train_loader:
        labels = batch['intensity_labels'].numpy()  
...
def objective(trial):
    ...
    set_seed(42)
    wandb.init(project="intensity_harmeme_hateClipperOptuna_new_Target_distil_hop2", mode="online", name=f"{device_id}_{trial.number}", 
    ...
    rootData = '../datasets/memes/defaults/annotations'
    imFolder = '../datasets/memes/defaults/images'

    train_dataset = HatefulMemesDataset(rootData, imFolder, split='train')
    val_dataset = HatefulMemesDataset(rootData, imFolder, split='val')
    test_dataset = HatefulMemesDataset(rootData, imFolder, split='test')
    ...
    class_weights_intensity = calculate_class_weights(train_loader)
    model = CLIPClassifier(gnnHidden=gnnHidden,weight =class_weights_intensity ,   gnnOutput=gnnOutput, ptFusion=ptFusion, graphFusion=graphFusion, fusionOutput=fusionOutput,
...
study_name = 'intensity_harmeme_hateClipperOptuna_new_Target_distil_hop2'
