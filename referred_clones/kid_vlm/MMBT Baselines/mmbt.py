# -*- coding: utf-8 -*-
"""mmbt-new-ocrcaptionsumm-500seq.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1N5Lu571Lg6w9bAQ0haSfLNo_eMW9kkIZ
"""
import os
...
class customMMBT(MMBTForClassification):
    def __init__(self, config, transformer, encoder, clfDropout=0.1):
        ...
        # print(f"Custom MMBT model initialized with enhanced cross-modal interaction (shrinking graph) with Relu with parameters: mhDropout: {mhDropout}, mhHeads: {mhHeads}, gnnHidden: {gnnHidden}, gnnOut: {gnnOut}, clfDropout: {clfDropout}", flush=True)
        # print(f"Fusion MMBT Checks with parameters: gnnHidden: {gnnHidden}, gnnOut: {gnnOut}, fusionType: {fusionType}, fusionOut: {fusionOut}, clfDropout: {clfDropout}", flush=True)
...
def collate_fn(batch):
    ...
    # Convert each graph in the batch to a Data object using your existing function
    # graph_data_list = [convert_to_rgcnn_data_with_transe(row["graph"], row["id"]) for row in batch]

    # Use PyTorch Geometric's Batch to handle a batch of graphs
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc=f"Training Epoch {i+1}")):
            ...
            if (step + 1) % eval_every == 0:
                ...
                if val_result['AUC'] > best_valid_auc:
                    ...
                    model_path = f'{file_path}/base-mmbt-model-embs-auc{best_valid_auc:.3f}-loss{val_loss:.3f}-acc{val_acc:.3f}.pt'
                    bestModel = model_path
                    torch.save(model.state_dict(), 'best_model_base_nocap.pth') 
...
def evaluate2(model, tokenizer, criterion, dataloader, device, tres=0.5):
    ...
    proba = None
    out_label_ids = None
    model.load_state_dict(torch.load('best_model_base_nocap.pth'))
...
def objective(trial):
    ...
    wandb.init(project=f"mmbt_runs", mode="online", name=f"mmbt_base_3")

    set_seed(42)
    print("Starting new trial")
    # print id
    # print("Trial number: {}".format(trial.number), flush=True)
    ...
    new_special_tokens = {
        "additional_special_tokens": ["<caption_start>", "<caption_end>", "<sep>"]
    }
