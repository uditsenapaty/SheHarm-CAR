...
triplesObj = {}
triplesFile = open('./closest_triples_with_names.jsonl', 'r', encoding='utf-8')
for line in triplesFile:
    obj = json.loads(line)
    triplesObj[int(obj['name'])] = obj['closest_triples']
...
class JsonlDataset(Dataset):
    def __init__(self, data_path, tokenizer, transforms, max_seq_length, image_encoder_size, device):
        ...
        for index in range(len(self.data)):
            ...
            triples_list = triplesObj.get(item_id, [])[:tripleK]
            triples_with_tags = ' '.join([f"<triple_start> {' '.join(triple)} <triple_end>" for triple in triples_list])
            combined_text = f"{item_text} {triples_with_tags}"
...
def load_examples(tokenizer, preprocess, max_seq_length, num_image_embeds, image_encoder_size, device, evaluate=0):
    ...
    if evaluate == 0:
        path = '../hateful_memes/train.jsonl'
    elif evaluate == 1:
        path = '../hateful_memes/dev_seen.jsonl'
    else:
        path = '../hateful_memes/test_seen.jsonl'
...
def train(model, tokenizer, criterion, optimizer, scheduler, train_dataloader, eval_dataloader, num_train_epochs, max_grad_norm, gradient_accumulation_steps, device, trial=None):
    ...
    running_loss = 0
    file_path = "../models/"
    model_best = "mmbt_triples_best.pt"
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc=f"Training Epoch {i+1}")):
            ...
            if (step + 1) % eval_every == 0:
                ...
                if val_result['AUC'] > best_valid_auc:
                    ...
                    model_path = f'{file_path}/triples-mmbt-model-embs-auc{best_valid_auc:.3f}-loss{val_loss:.3f}-acc{val_acc:.3f}.pt'
                    bestModel = model_path
                    torch.save(model.state_dict(), 'best_model_triples_nocap.pth') 
...
def evaluate2(model, tokenizer, criterion, dataloader, device, tres=0.5):
    ...
    proba = None
    out_label_ids = None
    model.load_state_dict(torch.load('best_model_triples_nocap.pth'))
...
def objective(trial):
    ...
    wandb.init(project=f"mmbt_runs", mode="online", name=f"mmbt_triples_3")
